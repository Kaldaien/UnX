...
unx::INI::File::File (wchar_t* filename)
{
  sections.clear ();
...
{
  unx::INI::File::Section section (name);
...
bool
Import_Section (unx::INI::File::Section& section, wchar_t* buffer, int start, int end)
{
  int key = start;
  for (int k = key; k < end; k++) {
    if (k < end - 1 && buffer [k] == L'=') {
...
void
unx::INI::File::import (std::wstring import_data)
{
...
std::wstring&
unx::INI::File::Section::get_value (std::wstring key)
{
...
bool
unx::INI::File::Section::contains_key (std::wstring key)
{
...
void
unx::INI::File::Section::add_key_value (std::wstring key, std::wstring value)
{
...
void
unx::INI::File::write (std::wstring fname)
{
  FILE*   fOut;
  errno_t ret;
...
const std::map <std::wstring, unx::INI::File::Section>&
unx::INI::File::get_sections (void)
{
  return sections;
}